...
class AudioAnalyzer:
    ...
    #     self.logger.info("Loading Silero VAD model...")
    #     try:
    #         # Force model download to a writable directory
    ...
    #             model='silero_vad',
    #             force_reload=True,
    #             trust_repo=True  # Add trust_repo flag
    #         )
    #         model = model.to(self.config.device)
    #         return model, utils
    #     except Exception as e:
    ...
    def _load_silero_vad(self):
        ...
        self.logger.info("Loading Silero VAD model...")
        try:
            # Create cache directory in /tmp for Streamlit Cloud
            cache_dir = "/tmp/.torch_hub"
            os.makedirs(cache_dir, exist_ok=True)
            torch.hub.set_dir(cache_dir)
            
            # Download model with specific source
            model, utils = torch.hub.load(
                repo_or_dir='snakers4/silero-vad',
                model='silero_vad',
                onnx=False,
            ...
            self.logger.error(f"Failed to load Silero VAD model: {str(e)}")
            self.logger.error(f"Cache directory contents: {os.listdir(cache_dir) if os.path.exists(cache_dir) else 'Directory not found'}")
            raise
